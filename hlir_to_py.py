...
def _args(xs: Iterable[str]) -> str:
    return ", ".join(xs)

def hlir_to_python(hlir: Dict[str, Any], alias: str = "VA", *, strict: bool = False, comment_unknowns: bool = True) -> str:
    """
    ...
    for op in hlir.get("program", []):
        ...
        if kind == "RECT":
            ...
            if comment_unknowns:
                lines.append(f"# TODO: unknown op dropped: {op!r}")
            continue
